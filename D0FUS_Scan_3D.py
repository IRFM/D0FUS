...
if chosen_parameter == 'R0':
    ...
    n_R0 = input("How many points for Pw : ")
    R0_values = np.linspace(2, 10, n_R0)
    
else :
    print("Wrong choice")

# Créer une grille 3D pour les valeurs de a, P_fus et P_W
a_grid, P_fus_grid, R0_grid = np.meshgrid(a_values, P_fus_values, R0_values, indexing='ij')
...
# scatter_radial = ax1.scatter(R0_grid.flatten(), P_fus_grid_radial.flatten(), a_grid.flatten(), c=radial_build_filtred.flatten(), cmap=color_radial_build,alpha=0.7,zorder=0)  #,s=1 
scatter_density = ax1.scatter(R0_grid.flatten(), P_fus_grid.flatten()/1e9, a_grid.flatten(), c=max_limits_density.flatten(), cmap=color_choice_density,vmin=min_colorbar,vmax=max_colorbar,alpha=1)
scatter_beta = ax1.scatter(R0_grid.flatten(), P_fus_grid.flatten()/1e9, a_grid.flatten(), c=max_limits_beta.flatten(), cmap=color_choice_beta,vmin=min_colorbar,vmax=max_colorbar,alpha=1)
scatter_security = ax1.scatter(R0_grid.flatten(), P_fus_grid.flatten()/1e9, a_grid.flatten(), c=max_limits_security.flatten(), cmap=color_choice_security,vmin=min_colorbar,vmax=max_colorbar,alpha=1)
scatter_power = ax1.scatter(R0_grid.flatten(), P_fus_grid.flatten()/1e9, a_grid.flatten(), c=max_limits_power.flatten(), cmap=color_choice_power,vmin=min_colorbar,vmax=max_colorbar,alpha=1)
...
ax1.set_xlabel('$R_0$ [m]', labelpad=10)
...
# Title
fig.text(0.55, 0.9, "Parameter space : a , $\mathbf{P_{fus}}$ , $\mathbf{R_{0}}$", fontsize=taille_titre, ha='center', va='center', fontweight='bold')
...
# Save the image
path_to_save = os.path.join(save_directory,f'Pfus_R0_a_scan_with_Bmax={int(Bmax)}_fp={f_RF_objectif*100}_scaling law={law}.png')
...
# Tracer les points en utilisant les valeurs calculées comme couleur
scatter = ax.scatter(R0_grid.flatten(), P_fus_grid.flatten(), a_grid.flatten(), c=radial_build_matrix.flatten(), cmap=color_choice_radial,alpha=0.5)
...
# Tracer les points en utilisant les valeurs calculées comme couleur
scatter = ax.scatter(R0_grid.flatten(), P_fus_grid.flatten(), a_grid.flatten(), c=max_limits_matrix.flatten(), cmap=color_choice_plasma,alpha=0.8)
...
ax.scatter(R0_points, P_fus_points, a_points, c='b', marker='o', label='max_limit ≈ 1')
