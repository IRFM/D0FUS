...
k_Boltzman = 1.6*10**-16 # Boltzman constant [J/keV]
...
A     = 2.5  # average atomic mass in AMU
...
Flux_CS_Utile = 0.85 # pourcentage disponible, laissant une partie libre pour le controle du plasma , arbitrairement à 0.85, la litterature propose plusieurs values allant de 0.85 à 1
q = 2
lower_bound_a = 0.3
upper_bound_a = 3
...
# C_SL = C_SL*(eta_T*E_F)/E_ALPHA # A creuser
# alpha_a = alpha_epsilon
# alpha_R = alpha_R-alpha_epsilon
...
def f_Ip(H,tauE,R0,a,κ,nbar,B0,A,eta_T,P_fus):
    ...
    Denominateur = H* C_SL * R0**alpha_R * Epsilon**alpha_epsilon * κ**alpha_kappa * (nbar*1.E-19)**alpha_n * partie_reelle * A**alpha_M * P_alpha**alpha_P
    inv_cI  = 1./alpha_I
    
    Ip = ((tauE/Denominateur)**inv_cI)*10**6 # in A
...
def f_beta_p_th(Cw,ne,Te,Ip,a):
    ...
    βp = 4/μ0 * Cw * ne * (Te*1000) * k_Boltzman / Ip**2 * L**2
    return(βp)
...
def f_plateau_duration(Bcs,R0,reCS,riCS,ne,Te,Ip,IBS,a):
    ...
    βp = 4/μ0 * Cw * L**2 * (ne*k_Boltzman*Te) / Ip**2 # 0.62 for ITER
    σp = conductivity(Te)
...
def joules_to_kev(energy_joules):
    # 1 Joule = 6.242e+15 kilo-electrovolts
...
def Solveur_parrallel(H,Bmax,P_fus,P_W,f_RF_objectif):
    delta = 0.002 # Finesse de la detection (en mètres)
    a_start = 0.4 # Valeur initiale de a 
    a_limite = 4.01 # Valeur max de a autorisee
    def To_solve_raffine(a):
        (R0_solution,B0_solution,pbar_solution,beta_solution,nbar_solution,tauE_solution,Ip_solution,qstar_solution,nG_solution,eta_CD_solution,fB_solution,fNC_solution,fRF_solution,n_vec_solution,c,cost,heat,solenoid,R0_a_solution,R0_a_b_solution,R0_a_b_c_solution,R0_a_b_c_CS_solution,required_Bcs) = calcul(a,H,Bmax,P_fus,P_W)
        return max(fRF_solution / f_RF_objectif ,n_vec_solution / nG_solution,beta_solution / betaN,q / qstar_solution)
    # Definir les limites de la recherche pour 'a'
    bounds = (lower_bound_a, upper_bound_a)
    # Minimiser la valeur maximale en utilisant minimize_scalar
    result = minimize_scalar(To_solve_raffine, bounds=bounds)
    # La valeur optimale de 'a' sera dans result.x
    optimal_a = result.x
    optimal_a_raffine = np.nan
    
    # Si nan : retour nan
    if To_solve_raffine(optimal_a) is None or np.isnan(To_solve_raffine(optimal_a)):
        return np.nan
    
    # Si L'une des 4 limites superieur à 1 alors on retourne a minisant le max des limites
    elif To_solve_raffine(optimal_a)>1:
            return optimal_a
    ...
    # Si il existe une solution :
        ...
        for a in np.arange(a_start,a_limite,delta) :
            # Evaluation des differentes valeurs utiles
            (R0_solution,B0_solution,pbar_solution,beta_solution,nbar_solution,tauE_solution,Ip_solution,qstar_solution,nG_solution,eta_CD_solution,fB_solution,fNC_solution,fRF_solution,n_vec_solution,c,cost,heat,solenoid,R0_a_solution,R0_a_b_solution,R0_a_b_c_solution,R0_a_b_c_CS_solution,required_Bcs) = calcul(a,H,Bmax,P_fus,P_W)
            cost = cost*1e6
            # Verifier si toutes les conditions sont satisfaites
            if (
                ...
                # Si oui :
                # Minimisation d'une fonction de cout souhaite, ici R0
...
def Solveur_raffine(H,Bmax,P_fus,P_W,f_RF_objectif):

    def To_solve_raffine(a):
        (R0_solution,B0_solution,pbar_solution,beta_solution,nbar_solution,tauE_solution,Ip_solution,qstar_solution,nG_solution,eta_CD_solution,fB_solution,fNC_solution,fRF_solution,n_vec_solution,c,cost,heat,solenoid,R0_a_solution,R0_a_b_solution,R0_a_b_c_solution,R0_a_b_c_CS_solution,required_Bcs) = calcul(a,H,Bmax,P_fus,P_W)
        return max(fRF_solution / f_RF_objectif ,n_vec_solution / nG_solution,beta_solution / betaN,q / qstar_solution)
    
    # Definir la fonction objectif à minimiser
    def objective_function_optimiseur(a):
        
        cost_fct = 0
        
        # Calculer les resultats pour le paramètre 'a' donne
        (R0_solution,B0_solution,pbar_solution,beta_solution,nbar_solution,tauE_solution,Ip_solution,qstar_solution,nG_solution,eta_CD_solution,fB_solution,fNC_solution,fRF_solution,n_vec_solution,c,cost,heat,solenoid,R0_a_solution,R0_a_b_solution,R0_a_b_c_solution,R0_a_b_c_CS_solution,required_Bcs) = calcul(a,H,Bmax,P_fus,P_W)
    ...
    bounds = (lower_bound_a, upper_bound_a)
    # Minimiser la valeur maximale en utilisant minimize_scalar
    result_minim = minimize_scalar(To_solve_raffine, bounds=bounds)
    
    # Verifier si l'optimisation a reussi et obtenir la valeur optimale de 'a'
    if result_minim.success:
        optimal_a = result_minim.x
    else:
        return np.nan  # Retourner NaN si minimize_scalar echoue
    ...
    # Si il existe une solution :
        ...
        # Definir les bornes pour le paramètre 'a' seulement
        bounds_a = [(lower_bound_a, upper_bound_a)]  # Limite pour 'a'

        # Utiliser la methode de Differential Evolution pour minimiser la fonction
        result_grad = differential_evolution(objective_function_optimiseur, bounds=bounds_a, maxiter=max_iterations, tol=1e-3)
        
            # Verifier si l'optimisation avec differential_evolution a reussi et obtenir la valeur optimale de 'a'
        if result_grad.success:
            optimal_a = result_grad.x[0]
        else:
            return np.nan  # Retourner NaN si differential_evolution echoue
...
def calcul(a, H, Bmax, P_fus, P_W):
    ...
    Ip_solution = f_Ip(H, f_tauE(pbar_solution, R0_solution, a, κ, eta_T, P_fus), R0_solution, a, κ, nbar_solution, B0_solution, A, eta_T, P_fus)
...
def find_required_reCS(R0,Ip,ne,Te,Bmax,a):
    ...
    try:
        # Utiliser root_scalar pour trouver la racine (zero) de la fonction total_flux
        result_re = root_scalar(total_flux_re, bracket=[0, 10], method='bisect', xtol=1e-2)
...
def find_required_Bcs(solenoid, R0_solution, Ip_solution, nbar_solution, Tbar, Bmax,a):
    
    reCS = solenoid
    R0 = R0_solution
    Ip = Ip_solution
    ne = nbar_solution
    Te = joules_to_kev(Tbar)
    ...
    try:
        # Utiliser root_scalar pour trouver la racine (zero) de la fonction total_flux
        result_B = root_scalar(total_flux_B, bracket=[0, Bmax], method='bisect', xtol=1e-2)
...
if parameter_values is not None:
    for parameter_value in tqdm(parameter_values, desc='Processing parameters'):
        ...
        # New solver
        a_solution = Solveur_raffine(H,Bmax,P_fus,P_W,f_RF_objectif)
...
bounds = [(0.3, 3),(0.4,1), (10, 24),(1e9,2e9),(0.5e6,4e6)] # a,H,Bmax,Pfus,Pw
...
Reactor = input("Entrez la machine que vous souhaitez etudier : (ITER,ARC,EUdemo_A3,EUdemo_A4,CFDTR,EUDEMO2,K_DEMO) or if you want to enter new value (New) :")
if Reactor == "New" :
    H = float(input("Entrez la valeur de H : "))
    P_fus = float(input("Entrez la valeur de P_F : "))
...
# Afficher le heatmap pour inverted_matrix_plasma_limit avec imshow et stocker l'objet mappable retourne
im_plasma_limit = plt.imshow(inverted_matrix_plasma_limit, cmap=color_choice_plasma, aspect='auto', interpolation='nearest') #lorsque plus de points disponibles, 'bilinear' possible afin d'interpoler de manière plus esthetique
# Definir la valeur seuil pour distinguer les deux plages de valeurs
threshold = 1.0
# Ajouter des contours pour marquer la ligne de demarcation pour inverted_matrix_plasma_limit
plt.contour(inverted_matrix_plasma_limit, levels=[threshold], colors='black', linestyles='dashed')
# Ajouter une barre de couleur avec une etiquette pour inverted_matrix_plasma_limit
cbar_plasma_limit = plt.colorbar(im_plasma_limit, label='max_limit')

# Limiter le nombre de valeurs affichees sur l'axe y
...
y_labels = [round(a_vec[len(a_vec) - 1 - i], 1) for i in y_indices]  # Arrondir chaque valeur à un decimale
plt.yticks(y_indices, y_labels)
# Limiter le nombre de valeurs affichees sur l'axe x
...
if chosen_parameter == 'Pfus':
    x_labels = [round(parameter_values[i]*1e-9, 1) for i in x_indices]  # Arrondir chaque valeur à une decimale
elif chosen_parameter == 'Pw':
    x_labels = [round(parameter_values[i]*1e-6, 1) for i in x_indices]  # Arrondir chaque valeur à une decimale
else :
    x_labels = [round(parameter_values[i], 1) for i in x_indices]  # Arrondir chaque valeur à une decimale
plt.xticks(x_indices, x_labels)

# # Afficher inverted_matrix_radial_build avec une colormap et stocker l'objet mappable retourne
...
# # Ajouter une colorbar avec une etiquette pour inverted_matrix_radial_build
# cbar_radial_build = plt.colorbar(im_radial_build, label='R0')

# Creer une copie de la matrice pour definir les niveaux de contour
...
# Definir les niveaux et les couleurs pour les contours
levels = np.arange(1, 25)  # Definit les niveaux de 1 à 24
# Ajouter des contours pour chaque niveau avec une couleur differente
contour_lines = plt.contour(inverted_matrix_radial_build, levels=levels, colors='white')
# Filtrer les niveaux pour n'afficher que ceux inferieurs à 10
filtered_levels = [level for level in contour_lines.levels if level <= 10]
# Ajouter les valeurs des niveaux à côte des contours
plt.clabel(contour_lines, filtered_levels, inline=True, fmt='%d', fontsize=8)
# Creer une ligne blanche avec une legende
white_line = mlines.Line2D([], [], color='white', label='Topological Map of R0')
# Ajouter la ligne à la legende
...
#%% Variation of Pfus and Pw while choosing the optimal a

# Définir les plages des variables a, P_fus et P_W
P_fus_values = np.linspace(1e9, 4e9,20)
P_W_values = np.linspace(1e6, 4e6, 20)
Bmax = 13

# Initialiser un tableau pour stocker les valeurs de max_limit
max_limit_grid = np.zeros((len(P_fus_values),len(P_W_values)))

# Calculer max_limit pour chaque combinaison de valeurs de a, P_fus et P_W
for i in tqdm(range(len(P_fus_values)), desc='Processing parameters'):
    for j in range(len(P_W_values)):
        
        P_fus = P_fus_values[i]
        P_W = P_W_values[j]
        
        a_optimal = Solveur_raffine(H,Bmax,P_fus,P_W,f_RF_objectif)
        ...
        (R0_solution, B0_solution, pbar_solution, beta_solution, nbar_solution,
         ...
         R0_a_b_c_CS_solution, required_Bcs) = calcul(a_optimal, H, Bmax,P_fus, P_W)
        ...
        # Stocker la valeur de max_limit dans le tableau
        max_limit_grid[i,j] = max_limit
...
im_plasma_limit = plt.imshow(max_limit_grid, cmap=color_choice_plasma, aspect='auto', interpolation='nearest') 
threshold = 1.0
plt.contour(max_limit_grid, levels=[threshold], colors='black', linestyles='dashed')
# Ajouter une barre de couleur avec une etiquette pour inverted_matrix_plasma_limit
cbar_plasma_limit = plt.colorbar(im_plasma_limit, label='max_limit')
...
# Save the image
path_to_save = os.path.join(save_directory,"Pfus_vs_Pw_map")
plt.savefig(path_to_save, dpi=300, bbox_inches='tight')
# Afficher la figure
plt.show()
...
# Générer des valeurs de rho de 0 à 1
rho_values = np.linspace(0, 1, 100)

# Calculer les profils de température et de densité
temperature_profile = f_Tprof(Tbar, nu_T, rho_values)
density_profile = f_nprof(nbar, nu_n, rho_values)

# Tracer les profils
plt.figure(figsize=(6, 6))

# Profil de température
plt.plot(rho_values, temperature_profile, label='Température', color='red')

# Ajouter des labels et une légende
plt.xlabel('Rho')
plt.ylabel('T')
plt.title('Profil de température en fonction de Rho')
plt.legend()
...
# Profil de densité
plt.plot(rho_values, density_profile, label='Densité', color='blue')

# Ajouter des labels et une légende
plt.xlabel('Rho')
plt.ylabel('ne')
plt.title('Profil de densité en fonction de Rho')
plt.legend()
...
# Créer une nouvelle figure sans échelles, grilles ni axes
plt.figure(figsize=(6, 6))
plt.axis('off')

# Tracer uniquement le profil de densité
plt.plot(rho_values, density_profile, color='blue')
